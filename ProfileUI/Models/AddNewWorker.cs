...
{
    /// <summary>
    /// A class that provides a method for sending a POST request to the server.
    /// </summary>
    class AddNewWorker
    {
        /// <summary>
        /// A method that generates an object of the Worker type from the required data
        /// and sends a POST request to the server at the ServerAdress address.Adress
        /// </summary>
        ...
        public bool Add(string name, string firstName, string father, DateTime date,
                        string adress, string otdel, string about)
        {
            using (var client = new HttpClient())
            {
                ...
                var result = client.PostAsync($"/api/profile", new StringContent(myJson, Encoding.UTF8, "application/json")).Result;
